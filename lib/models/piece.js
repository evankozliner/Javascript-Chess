...
(function() {
  ...
  function getMoves(range, piece, columns) {
    ...
    if ( piece.type === 'pawn' ) {
      ...
      rightDiagonal = columns[columns.indexOf(piece.column) + 1] + ( Number(piece.row) + 1 * multiplier );
      leftDiagonal = columns[columns.indexOf(piece.column) - 1] + ( Number(piece.row) + 1 * multiplier );

      if ( nextNorth.childNodes.length > 0 )  {
        moves.splice(moves.indexOf(nextNorth));
      }
      if ( document.getElementById(rightDiagonal).childNodes.length > 0) {
        moves.push(rightDiagonal);
      }
      if ( document.getElementById(leftDiagonal).childNodes.length > 0 ) {
