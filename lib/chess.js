...
(function() {
  ...
  ls.setItem('aGameOfChess', JSON.stringify({pieces: pieces}));
  var chess = JSON.parse(ls.getItem('aGameOfChess'));
 ...
 */
function getPieces() {
  return (JSON.parse(window.localStorage.getItem('aGameOfChess'))).pieces;
}
...
function setPieces(pieces) {
  var state = JSON.parse(window.localStorage.getItem('aGameOfChess'));
  state.pieces = pieces;
